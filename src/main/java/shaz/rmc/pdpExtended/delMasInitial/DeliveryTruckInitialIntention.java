...
public class DeliveryTruckInitialIntention {
	...
	public void followSchedule(TimeLapse time) {
		...
		newFollowSchedule(time);
	}
	
	private void newFollowSchedule(TimeLapse time) {
		...
		if (isInCurrentUnitTime(currentTime)) { //remain in current unit		
				if (b.schedule.get(currentUnitNo) instanceof TruckDeliveryUnit) {
					TruckDeliveryUnit currUnit = (TruckDeliveryUnit)b.schedule.get(currentUnitNo);
					Delivery currentDelivery = ((TruckDeliveryUnit)b.schedule.get(currentUnitNo)).getDelivery();
					...
					TruckTravelUnit currUnit = (TruckTravelUnit)b.schedule.get(currentUnitNo);
			...
			if (currentTime >= b.schedule.get(currentUnitNo+1).getTimeSlot().getStartTime().getMillis()) {
				...
				checkArgument(b.schedule.get(currentUnitNo).getEndLocation().equals(b.schedule.get(currentUnitNo+1).getStartLocation()), true);
				checkArgument(rmcTruck.getRoadModel().getPosition(rmcTruck).equals(b.schedule.get(currentUnitNo+1).getStartLocation()), true);
				currentUnitNo++;
				doneCurrentUnit = false;
			}
	}
	 ...
	 * @return
	 */
	private boolean isInCurrentUnitTime(long currentTime) {
		return currentTime >= b.schedule.get(currentUnitNo).getTimeSlot().getStartTime().getMillis() //if current time is equal to trucks currentUnit's starttime
				&& currentTime <= b.schedule.get(currentUnitNo).getTimeSlot().getEndTime().plusMillis(500).getMillis();
	}
