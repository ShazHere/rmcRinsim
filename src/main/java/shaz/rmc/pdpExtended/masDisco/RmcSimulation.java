...
public class RmcSimulation {
	...
	public static void main(String[] args) {
		 ...
		 * */
		PlaneRoadModel prm = new PlaneRoadModel(new Point(0, 0), new Point(10, 10), false, 1);//10by10 km plane
		...
		for (int j = 0; j<3 ; j++) 
			sim.register(new RmcProductionSite(GlobalParameters.PROBLEM.getStations().get(j)));
		
		//Adding orders
		for (int i = 0; i< 6; i ++){
			sim.register(new RmcOrderAgent(sim, prm.getRandomPosition(new MersenneTwister()), GlobalParameters.PROBLEM.getOrders().get(i)));		
		}
		
		//Adding Delivery Trucks
		for (int i = 0; i< 15 /*GlobalParameters.TOTAL_TRUCKS*/ ; i ++){
			sim.register(new RmcDeliveryTruck(prm.getRandomPosition(rng), rmSim.getTruck(i), sim));
		}
		...
		schema.add(OrderSite.class,"/graphics/perspective/tall-building-32.png"); 
	 ...
	 * 
	 */
	private void loadProblem(Simulator sim) {
