 ...
 *
 */
public class RmcDeliveryTruck extends rinde.sim.core.model.pdp.Vehicle implements MovingRoadUser, Agent {
	//protected Point startLocation;
	private final RandomData randomPCSelector;
	private final Mailbox mailbox;
	private CommunicationAPI cApi;
	private final Simulator sim;
	private static Logger log = Logger.getLogger(RmcDeliveryTruck.class);
	...
	private final RmcDeliveryTruckIntention i;
	...
	public RmcDeliveryTruck (Point pStartLocation, shaz.rmc.core.domain.Vehicle pTruck, Simulator pSim){
		sim = pSim;
		...
		i = new RmcDeliveryTruckIntention(this, b, schedule);
		explorationCount = 0; 
	...
	protected void tickImpl(TimeLapse time) {
		...
		if (!schedule.isEmpty()) {
			assert schedule.get(schedule.size()-1).getTimeSlot().getProductionSiteAtStartTime().getStation() != null : truck.getId()+"T: The return location of Truck shouldn't be null";
			...
			for (final OrderSite p : orderSites) { //get the one with smallest distance
				...
				if (dist <= selectedDist) {
					if (!isProposed(p.getOrder()) && p.getOrder().isOrderReserved() == false) { //propose only if order isn't reserved
						...
						ExplorationAnt expAnt = new ExplorationAnt(this,  time.getStartTime());
		...
		if ((!b.explorationAnts.isEmpty())) {//&& continueExploration == true) {
			...
			while (i.hasNext()) { //out of these one best should be selected and proposed
				...
				if (time.getStartTime() == exp.getCreatedTime() * GlobalParameters.EXPLORATION_EVAPORATION_RATE ) {
					log.debug(this.getId()+ "T: evaporated Exp Ant & CURRENT TIME IS = " + new DateTime(GlobalParameters.START_DATETIME.getMillis() + time.getStartTime()));
					i.remove();
				}
				if (exp.isReturned() && exp.getDelivery() != null) { //returned with delivery
					...
					if (startTime == null)  {//means station isn't free afterwards..
						...
						log.debug(this.getId()+ "T: Intention ant sent to " + ((RmcOrderAgent)exp.getOrder()).getOrder().getId() +"O & CURRENT TIME IS = " + new DateTime(GlobalParameters.START_DATETIME.getMillis() + time.getStartTime()));
					...
					log.debug(this.getId()+ "T: exp from "+ ((RmcOrderAgent)exp.getOrder()).getOrder().getId()+ "O returned without Delivery & CURRENT TIME IS = " + new DateTime(GlobalParameters.START_DATETIME.getMillis() + time.getStartTime()));
					i.remove();
				}
			}
		}
		...
		if(!b.intentionAnts.isEmpty()) {
			...
			while (i.hasNext())  {
				...
				if (intAnt.getReply() == Reply.NO_REPLY || intAnt.getReply() == Reply.UNDER_PROCESS)
					...
					double distance = Point.distance(roadModel.getPosition(intAnt.getIntendedTimeSlot().getProductionSiteAtStartTime()), ((RmcOrderAgent)intAnt.getOrder()).getPosition());
					d.setStationToCYTravelTime(new Duration ((int)(distance/this.getSpeed()*3600)));//roughly this shud b traveling time
					...
					distance =Point.distance(((RmcOrderAgent)intAnt.getOrder()).getPosition(), sites.get(rand).getLocation()); //Only one PC is assumed
					d.setCYToStationTravelTime(new Duration ((int)(distance/this.getSpeed()*3600)));
					...
					//log.debug(this.getId()+ "T: STAtionto CYTravelTime = " + d.getStationToCYTravelTime());
					//log.debug(this.getId()+ "T: CY to STation TravelTime = " + d.getCYToStationTravelTime());
...
//		}
	...
	/**
	 * @param ts: TimeSlot is given considering it as end of the last scheduled unit.  
	 * @return : True if available slot of the truck is changed, false if slot is not changed. 
	 * @Description:  We keep the pattern of truck travel, that it is on a PC and proposes the order with its PC location at start time of available slot.
	 * So the travel cycle becomes, StartPCLocation -> nextOrder -> randomPC -> nextOrder -> randomPC. Latter we can select nearest PC instead of random PC.
	 * 
	 */
	public boolean arrangeSlots(TimeSlot ts) {	
		if (b.getAvailableslot().compareTo(new TimeSlot(ts.getEndTime(), b.getAvailableslot().getEndTime())) == 1)  { //means availableSlot should be changed because both values are not equal
			log.debug(this.getId()+"T: Available slot changed, PREVIOUS slot = start:" + b.getAvailableslot().getStartTime() 
				+ ", & end:" + b.getAvailableslot().getEndTime() + "& NEW slot = start:" + ts.getEndTime() + " & end:" + b.getAvailableslot().getEndTime() +
				" and PC = " + ts.getProductionSiteAtStartTime().getStation());
			b.setAvailableslot(ts.getEndTime(), b.getAvailableslot().getEndTime(), ts.getLocationAtStartTime(), ts.getProductionSiteAtStartTime());
			return true;
		}
		return false;
	}
	...
	public double getSpeed() {
		return .03;
	}// As per my exploration, this speed is in units per hour. So the truck will travel 0.03/hour. 
